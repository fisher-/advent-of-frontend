...
describe("GiftRegistry", () => {
  ...
  it("should not duplicate gifts for a given child", () => {
    const registry = new GiftRegistry();
    registry.addGift(1, "teddy bear");
    registry.addGift(1, "teddy bear");
    expect(registry.getGiftsForChild(1)).toEqual(["teddy bear"]);
  });
  ...
  it("should throw an error if trying to remove a gift from a child that does not exist", () => {
    const registry = new GiftRegistry();
    expect(() => registry.removeGift(1, "puzzle")).toThrow("Child not found");
  });
